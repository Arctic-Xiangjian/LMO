...
class IXI_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image_rec = np.load(os.path.join(self.space_data_dir_rec, name_rec)) # 清晰图像
        image_ref = np.load(os.path.join(self.space_data_dir_ref, name_ref)) # 辅助图像
        mask = np.load(self.mask_path) # mask
        ...
        return (name_rec, # name 
                image_rec, # image
                fully, # [real,imag]
                under_sample, # [under_sample_real, under_sample_imag]
                mask, # mask 
                name_ref, # name 
                image_ref, # image
                fully_ref) # [real,imag]
