...
def train(train_loader, model, loss, optimizer, epoch, train_writer, device, logger, args):
    ...
    for idx, (name_rec, image_rec, fully, under_sample, mask, name_ref, image_ref, fully_ref) in enumerate(train_loader):
        ...
        image_rec = image_rec.float().to(device)  # (bs, 1, 256, 256)  GT
        fully = fully.float().to(device)  # (bs, 2, 256, 256)  GT_fourier: real imag
        under_sample = under_sample.float().to(device)  # (bs, 2, 256, 256) # GT_fourier@ mask
        mask = mask.float().to(device)  # (bs, 1, 256, 256) # mask 
        ...
        under_image_rec = torch.abs(torch.fft.ifft2(under_sample)) # image

        lossc = CharbonnierLoss().to(device)
        output = model(under_image_rec, under_sample, mask) # 
        loss_ = lossc(output, image_rec)
    ...
    with torch.no_grad():
        for idx, (name_rec, image_rec, fully, under_sample, mask, name_ref, image_ref, fully_ref) in tqdm(enumerate(valid_loader), desc='Batch',
            ...
            psnr = peak_signal_noise_ratio(np.array(image_rec[0, 0, :, :].data.cpu()),
                                           np.array(output[0, 0, :, :].clamp(0, 1).data.cpu()),
